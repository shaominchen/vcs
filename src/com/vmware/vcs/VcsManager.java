...
import com.vmware.vcs.util.CustomProxySelector;
import com.vmware.vcs.util.EamConnection;
import com.vmware.vcs.util.KeepConnectionAlive;
import com.vmware.vcs.util.VimConnection;
...
/**
 * Main manager class in the vSphere Container Service. This class initializes and sets up
 * all the objects.
 */
public class VcsManager implements InitializingBean {

   /** Solution's extension key */
   public final String EXTENSION_KEY;

   /** Logger */
   private static Logger _logger = Logger.getLogger(VcsManager.class.getName());

   /**
    * TLS version string for getting SSLContext
    */
   private static final String TLS12 = "TLSv1.2";

   /** Singleton VcsManager instance */
   private static VcsManager _instance;
   ...
   public VcsManager(String extensionKey,
                  String selfUrl,
                  String selfIp,
                  String vcUsername,
                  String vcPassword,
      ...
      synchronized (this) {
         if (_instance != null) {
            throw new Exception("VcsManager already initialized!");
         }
         _instance = this;
      }
   ...
   private void initializeSsl() throws KeyStoreException, UnrecoverableKeyException,
      ...
      InputStream is = getClass().getClassLoader().getResourceAsStream("/vcs.keystore");
      ks.load(is, password);
      is.close();
      ...
      try {
         SSLContext sc = SSLContext.getInstance(TLS12);
         ...
         throw new IOException("Secure socket configuration failed.", e);
      }
   }
   ...
   /**
    * Initialize the VCS solution, called after all spring initialization is done.
    *
    * 1. Register as vCenter Extension 2. Connect to VCS 3. Create agency (setup
    * agent handler)
    */
   @Override
   ...
   private Extension createExtensionObject() {
      ...
      description.setLabel("VCS Solution");
      description.setSummary("This extension provides the VCS Solution.");
      ...
      keyValue.setValue("VCS Solution");
